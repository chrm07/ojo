const app = {
            ...
            try {
                ...
                matchedProjs.forEach(item => {
                    ...
                    <div class="search-result-item" onclick="app.showModule('projects'); setTimeout(() => app.openProjectDetails(${item.id}, '${safe(item.name)}', '${safe(item.location)}'), 250)">
                        ...
                        <div class="search-content">
                            <h4>${item.name || 'Unnamed Project'}</h4>
                            <p>${item.location || 'No location'} | Foreman: ${item.foreman || 'N/A'}</p>
                ...
                matchedUsers.forEach(item => {
                    ...
                    <div class="search-result-item" onclick="app.showModule('users'); setTimeout(() => app.openSkillFolder('${safeSkill}'), 250)">
                        ...
                        <div class="search-content">
                            <h4>${item.name || 'Unnamed Worker'}</h4>
                            <p>${item.position || 'Worker'} | ${item.skills || 'N/A'}</p>
                ...
                matchedSuppliers.forEach(item => {
                    ...
                    <div class="search-result-item" onclick="app.showModule('materials'); setTimeout(() => app.switchMatTab('suppliers'), 250);">
                        ...
                        <div class="search-content">
                            <h4>${item.name || 'Unnamed Supplier'}</h4>
                            <p>Provides: ${item.materials || 'N/A'}</p>
                ...
                matchedInventory.forEach(item => {
                    ...
                    <div class="search-result-item" onclick="app.showModule('materials'); setTimeout(() => app.switchMatTab('inventory'), 250);">
                        ...
                        <div class="search-content">
                            ...
                            <p>Stock: ${item.stock || 0} ${item.unit || ''} | Category: ${item.category || 'N/A'}</p>
                ...
                matchedAwards.forEach(item => {
                    ...
                    <div class="search-result-item" onclick="app.showModule('award_costs');">
                        ...
                        <div class="search-content">
                            <h4>${item.scope_of_work || 'Award Cost'}</h4>
                            <p>Amount: ₱${parseFloat(item.amount || 0).toLocaleString('en-US')}</p>
                ...
                matchedCash.forEach(item => {
                    ...
                    <div class="search-result-item" onclick="app.showModule('cash_release');">
                        ...
                        <div class="search-content">
                            <h4>${item.name || 'Cash Release'} - ${item.category || 'N/A'}</h4>
                            <p>${item.description || 'No Description'} | Amount: ₱${parseFloat(item.amount || 0).toLocaleString('en-US')}</p>
    ...
    addManpower: async function () {
        ...
        if (skills === 'ADD_NEW') {
            skills = (skillsNewInput?.value || '').trim();
        }

        let position = (positionInput?.value || '').trim();
        if (position === 'ADD_NEW') {
            position = (positionNewInput?.value || '').trim();
        }
        ...
        if (res.status === 'success') {
            ...
            if (skillsNewInput) {
                skillsNewInput.value = '';
                skillsNewInput.style.display = 'none';
            }

            if (positionInput) positionInput.value = '';
            if (positionNewInput) {
                positionNewInput.value = '';
                positionNewInput.style.display = 'none';
            }
            ...
            await this.populateManpowerDropdowns();
            await this.loadManpowerFolders();

            if (typeof this.loadDashboard === 'function') {
                this.loadDashboard();
            }

            if (typeof this.populateForemanDropdown === 'function') {
                this.populateForemanDropdown();
            }

            this.showToast("Record successfully added.");
        } else {
            this.showToast("Warning: " + (res.message || 'Unable to add record.'), 'error');
        }
    },
    ...
    loadManpowerFolders: async function (page = 1) {
        ...
        grid.innerHTML += `
        <div class="stat-card" style="cursor:pointer; background:#FEF2F2; border-color:#FCA5A5; min-height: 100px;" onclick="app.openArchivedFolder()">
            <div class="stat-details">
                <h3 style="font-size: 1rem; color:var(--danger); font-weight:800;">
                    <i class="fa-solid fa-box-archive" style="margin-right: 4px;"></i> Archived Records
                </h3>
                <span style="color:var(--text-muted); font-size:0.85rem;">Inactive Worker Pool</span>
            </div>
        </div>
    `;
    },
    ...
    uploadBioData: async function (workerId, inputElement) {
        ...
        if (res.status === 'success') {
            ...
            const currentTitle = document.getElementById('current-skill-title');
            if (currentTitle) {
                const currentTitleRaw = currentTitle.innerText.trim();
                const cleanTitle = currentTitleRaw.replace('Rename Folder', '').replace('Delete Folder', '').trim();

                if (cleanTitle) {
                    this.openSkillFolder(cleanTitle);
                } else {
            ...
            this.showToast(res.message || 'Unable to update bio data.', 'error');
        }
    },
