...
class ConstructionSystem {
    ...
    // --- CHECKLIST CRUD ---
    public function addChecklistTask($project_id, $category, $task_name) { $this->pdo->prepare("INSERT INTO project_accomplishments (project_id, category, task_name, status) VALUES (?, ?, ?, 'Not Started')")->execute([$project_id, $category, $task_name]); return ['status' => 'success']; }
    ...
    public function issueMaterial($project_id, $item_id, $qty, $receiver) {
        ...
        } catch(Exception $e) { $this->pdo->rollBack(); return ['status' => 'error', 'message' => 'Transaction failed.']; }
    }
    ...
    public function getManpowerBySkill($skill) { 
        $stmt = $this->pdo->prepare("SELECT m.*, p.name as project_name FROM manpower m LEFT JOIN projects p ON m.project_id = p.id WHERE TRIM(m.skills) = ? AND m.is_archived = 0 ORDER BY m.name ASC"); 
        $stmt->execute([trim($skill)]); return $stmt->fetchAll(PDO::FETCH_ASSOC); 
    }
    public function addManpower($name, $skills, $position, $salary, $project_id, $photo) {
        ...
        $this->pdo->prepare("INSERT INTO manpower (name, skills, position, rate, project_id, photo_path) VALUES (?, ?, ?, ?, ?, ?)")
                  ->execute([$name, $skills, $position, $salary, $project_id ?: null, $filePath]);
        return ['status' => 'success'];
    }
    ...
    // --- PAYROLL ---
    public function getPayroll() { return $this->pdo->query("SELECT * FROM payroll")->fetchAll(PDO::FETCH_ASSOC); }
    public function getPayrollHistory() { return $this->pdo->query("SELECT * FROM payroll_history ORDER BY id DESC")->fetchAll(PDO::FETCH_ASSOC); }
    public function addPayroll($date, $name, $job, $days, $deduc) {
        $stmt = $this->pdo->prepare("SELECT rate FROM manpower WHERE name = ? LIMIT 1"); $stmt->execute([$name]);
        $rate = $stmt->fetchColumn() ?: 0;
        $gross = $rate * $days; $net = $gross - $deduc;
        $this->pdo->prepare("INSERT INTO payroll (pay_date, name, job_description, rate, days_worked, gross_pay, deductions, net_pay) VALUES (?, ?, ?, ?, ?, ?, ?, ?)")->execute([$date, $name, $job, $rate, $days, $gross, $deduc, $net]);
        return ['status' => 'success'];
    }
    public function archiveAndResetPayroll() {
        $cycle = 'CYCLE-' . date('Ymd-His');
        $payroll = $this->getPayroll();
        $stmt = $this->pdo->prepare("INSERT INTO payroll_history (cycle_id, pay_date, name, job_description, rate, net_pay) VALUES (?, ?, ?, ?, ?, ?)");
        foreach($payroll as $p) { $stmt->execute([$cycle, $p['pay_date'], $p['name'], $p['job_description'], $p['rate'], $p['net_pay']]); }
    ...
    public function uploadNTPFile($project_id, $ticket, $date, $award_cost, $due_date, $accept_date, $file) {
        ...
        if (move_uploaded_file($file['tmp_name'], '../' . $filePath)) {
            $this->pdo->prepare("INSERT INTO project_ntp (project_id, ntp_ticket, date_received, award_cost, due_date, acceptance_date, file_path) VALUES (?, ?, ?, ?, ?, ?, ?)")
                      ->execute([$project_id, $ticket, $date, $award_cost, $due_date, $accept_date, $filePath]);
